<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" href="{{ asset('site/images/favicon.ico')}}">
    <title>@yield('title','Post Card') </title>
    <meta name="description" content="@yield('description','Post Card')" />
  <meta name="keywords" content="@yield('keywords','Post Card')" />
    <!-- Bootstrap core CSS -->
    <link href="{{ asset('site/css/bootstrap.min.css')}} " rel="stylesheet">
    
    <!-- Fonts CSS -->
    <link href="{{ asset('site/css/fonts.css')}}" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Roboto:300,400,500,700,900&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Lexend+Exa&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Roboto+Mono:300,400,500,700&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Libre+Baskerville&display=swap" rel="stylesheet">
    <!-- Dragbox Css -->
    <link rel="stylesheet" href="{{ asset('site/css/jquery-ui.css')}}">
    <!-- Slider Css -->
    <link rel='stylesheet' type="text/css" href="{{ asset('site/css/owl.carousel.min.css')}}" >
    <link rel='stylesheet' type="text/css" href="{{ asset('site/css/owl.theme.default.min.css')}}" >
    <link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/css/toastr.css">
    <link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/css/toastr.css">
    <!-- Custom styles -->
    <link href="{{ asset('site/css/style.css')}}" rel="stylesheet" type="text/css">
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-159396356-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-159396356-1');
</script>

</head>
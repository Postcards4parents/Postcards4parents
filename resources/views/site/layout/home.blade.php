...
@php
  ...
  $message=$welcomeMsg->get('moduleText1');
  if(!empty($message)){
    ...
    <div class="row align-items-center">
      <div class="col-md-6 text-center">
        <div class="about-video m-auto">
          <iframe width="100%" height="300" src="https://www.youtube.com/embed/{{$vedioLink2[1]}}?loop=1&playlist={{$vedioLink2[1]}}&showinfo=0&controls=0" frameborder="0" allowfullscreen></iframe>
        </div>
      </div>
...
<section class="p4p clearfix">
  <div class="container">
    <h2>Why Postcards for Parents?</h2> 
    <div class="row text-center">
      @foreach($productBenefit as $key=>$productBenefitList)
        @php
        $Icon = $productBenefitList->get('productBenefitIcon', null, false);
        if(!empty($Icon)){
          $Icon = $client->resolveLink($Icon);
          $Iconurl= $Icon->getFile()->getUrl();
        }else{
          $Iconurl="";
        }
        @endphp
        <div class="col-sm-3 col-md-2 mb-2 mb-md-0">
          <div class="p4picon rounded-circle">
            <img src="{{$Iconurl}}" alt="" class="img-fluid rounded-circle">
          </div>
          <p>{{$productBenefitList->productBenefit}}</p>
        </div>
      @endforeach
    </div>
  </div>

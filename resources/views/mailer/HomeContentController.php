...
public function newSubscribers(){
    ...
    ->select('offer_payemnt_tbl.*', 'user_quiz.grade')->where('created_at','>','NOW() - INTERVAL 1 DAY')
    ->get()->toArray();
    
        $query1 = $this
                    ->query
                    ->setContentType("EmailExtra")->where('sys.id','64EdnPdIMB1GKeCB5F30py');
        $checkwelcomekit = $this
                ->client
                ->getEntries($query1)[0];
        $query2 = $this
                ->query
                ->setContentType("EmailExtra")->where('sys.id','1MwLNwh5e6k5L3DVUjZmNZ');
    $quizTake = $this
            ->client
            ->getEntries($query1)[0];
                
        $emailSubjectLine1 = $checkwelcomekit->emailSubject;
        $emailSubjectLine2 = $quizTake->emailSubject;
        ...
        foreach ($data as $user)
                {
                    ...
                    if($user['grade'] !== Null){
                        try
                        {
                            Mail::send('mailer.welcomekitcheck', ["data" => $checkwelcomekit, 'fname' => $user['user_name']], function ($message) use ($user, $emailSubjectLine1)
                            {
                        ...
                        try
                        {
                            Mail::send('mailer.takequiz', ["data" => $quizTake, 'fname' => $user['fname']], function ($message) use ($user, $emailSubjectLine2)
                            {
...
public function quizTakersNew(){
    $data = DB::table('user_quiz')->join('offer_payemnt_tbl', 'offer_payemnt_tbl.user_id', '=', 'offer_payemnt_tbl.user_id', 'left')
    ->select('user_quiz.*', 'offer_payemnt_tbl.subscription_id')->where('user_quiz.created_at','<','NOW() - INTERVAL 1 DAY')->groupBy('user_quiz.user_email')
    ->get()->toArray();
